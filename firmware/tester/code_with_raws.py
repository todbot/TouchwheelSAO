...
# but = digitalio.DigitalInOut(board.GP8)
# but.switch_to_input(pull=digitalio.Pull.UP)
...
lbl_pos_text = label.Label(terminalio.FONT, text="pos:", x=0, y=25, scale=1)
lbl_pos_val = label.Label(terminalio.FONT, text="unk", x=30, y=25, scale=1)
lbl_touches_text = label.Label(terminalio.FONT, text="touches:", x=0, y=40, scale=1)
lbl_touches_val = label.Label(terminalio.FONT, text="   ", x=60, y=40, scale=1)
lbl_rawtouch_text = label.Label(terminalio.FONT, text="raw_val:", x=0, y=55, scale=1)
lbl_rawtouch0_val = label.Label(terminalio.FONT, text="00", x=60, y=55, scale=1)
lbl_rawtouch1_val = label.Label(terminalio.FONT, text="00", x=80, y=55, scale=1)
lbl_rawtouch2_val = label.Label(terminalio.FONT, text="00", x=100, y=55, scale=1)
for l in (lbl_title, lbl_pos_text, lbl_pos_val, lbl_touches_text, lbl_touches_val,
          lbl_rawtouch_text, lbl_rawtouch0_val, lbl_rawtouch1_val, lbl_rawtouch2_val):
    maingroup.append(l)
...
while True:
    ...
    try:
        ...
        print("  touches:", f'{r[0]:03b}')
        lbl_touches_val.text = f'{r[0]:03b}'

        r = i2c_read_register_bytes(sao_i2cdev, 2, 1)
        print("  raw_touch0:", f'{r[0]:02x}')
        lbl_rawtouch0_val.text = f'{r[0]:02x}'
        
        r = i2c_read_register_bytes(sao_i2cdev, 4, 1)
        print("  raw_touch1:", f'{r[0]:02x}')
        lbl_rawtouch1_val.text = f'{r[0]:02x}'
        
        r = i2c_read_register_bytes(sao_i2cdev, 6, 1)
        print("  raw_touch0:", f'{r[0]:02x}')
        lbl_rawtouch2_val.text = f'{r[0]:02x}'
